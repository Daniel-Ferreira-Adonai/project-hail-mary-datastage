...
private EncounterData _GetRandomEncounterForRow(int row)
{
    ...
    if (row <= 4)
        ...
        pool = MediumEncounters.Length > 0 ? MediumEncounters : EasyEncounters;
    else
        pool = HardEncounters.Length > 0 ? HardEncounters : (MediumEncounters.Length > 0 ? MediumEncounters : EasyEncounters);
