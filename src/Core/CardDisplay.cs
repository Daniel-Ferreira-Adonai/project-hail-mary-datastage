...
{
    public enum CardDisplayContext { Reward, Shop, DeckViewer }

    [Export] SubViewport _viewport;
    [Export] public CardDisplayContext Context { get; set; } = CardDisplayContext.Reward;
...
private void OnMouseExited()
{
    ...
    public void UpdatePreview(Player player)
    {
        ...
        _viewport ??= GetNode<SubViewport>("SubViewport");
        var card = _viewport.GetChild<Card>(0);
        card.UpdateDamagePreview(player);
	...
	private void OnGuiInput(InputEvent @event)
	{
		...
		mouse.Pressed)
		{
            ...
            if (Context == CardDisplayContext.Reward)
            {
			    PlayChosenAnimation();
            }
		}
